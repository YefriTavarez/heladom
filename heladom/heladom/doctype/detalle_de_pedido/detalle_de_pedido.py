...
class DetalledePedido(Document):
	def calculate_dates(self):
		...
		self.recent_history_current_year_start_date = add_weeks(self.date, -cutoff_trend + 1) # usually 2 weeks back
		...
		if transit_weeks:
			...
			self.transit_period_start_date = add_days(self.recent_history_last_year_end_date, +WEEK_DAYS)
			self.transit_period_end_date = self.transit_period_start_date
	...
	def set_missing_values(self):
		...
		from heladom.api import get_last_stock
		...
		self.order_sku_existency = get_last_stock(self.date, self.sku, self.cost_center)

		tendency__use_period = float(self.presup_gral) / 100.000
		...
		##### SECCION ORDEN FINAL ######
		from heladom.api import get_total_not_received
		...
		self.order_sku_in_transit = get_total_not_received(self.sku, self.cost_center)
	...
	def fill_tables(self):
		...
		for trend_week in trend_date_as_array:
			if trend_weeks:
				self.append("current_period_table", 
					self.get_physical_stock_as_dict(trend_week, self.sku, self.cost_center)
				)
		...
		for previous_week in prev_date_as_array:  
			if trend_weeks:
				self.append("previous_period_table",
					self.get_physical_stock_as_dict(previous_week, self.sku, self.cost_center)
				)
		...
		for transit_week in transit_date_as_array:
			if transit_weeks:
				self.append("transit_period_table", 
					self.get_physical_stock_as_dict(transit_week, self.sku, self.cost_center)
				)
		...
		for usage_week in usage_date_as_array:
			if consumption_weeks:
				self.append("usage_period_table", 
					self.get_physical_stock_as_dict(usage_week, self.sku, self.cost_center)
				)

	def get_physical_stock_as_dict(self, date, sku, cost_center):
		...
		row = frappe.db.sql("""SELECT fecha_transacion AS ciclo, consumo AS desp, existencia AS exist
			...
			AND centro_de_costo = '%(centro_de_costo)s'""" 
		% {"date": date, "sku" : sku, "centro_de_costo": cost_center}, as_dict=True)

		if not row:
			return { "ciclo": '', "desp": 0, "exist":0 }
