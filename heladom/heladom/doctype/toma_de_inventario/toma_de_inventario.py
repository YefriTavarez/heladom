...
def register(items, toma_nombre):
    for item in items:
        cantidad_actual = frappe.get_value('Stock Ledger Entry', filters={
        ...
        record = frappe.get_doc({
            ...
            "company": "Heladom",
            "sku": item["item_code"],
            "generico": frappe.get_value(
                'Item',
                filters={'item_code': item["item_code"]},
                fieldname=['generic']
            ),
            "clave_de_rotacion": frappe.get_value(
                'Item',
                filters={'item_code': item["item_code"]},
                fieldname=['rotation_key']
            ),
            "insumo": frappe.get_value(
                'Item',
                filters={'item_code': item["item_code"]},
                fieldname=['insumo']
            ),
            ...
            "unidad_minima": frappe.get_value(
                'Item',
                filters={'item_code': item["item_code"]},
                fieldname=['stock_uom']
            ),
...
def create_material_issue(items):
    item_table = []
    for item in items:
        cantidad_actual = frappe.get_value(
            'Stock Ledger Entry',
            filters={
                'item_code': item["item_code"],
                'warehouse': item["warehouse"]
...
def group_items_insumo(item_table):
    ...
    for item in groups.values():
        ...
        valuation_rate = frappe.get_all(
            'Stock Ledger Entry',
            filters={'item_code': item[0].item,
...
def as_insumo(item_list_sku):
    ...
    for item_sku in item_list_sku:
        item_insumo_dic = {
            ...
            "conversion": frappe.get_value('Item',
                                           item_sku["item_code"],
                                           'conversion_ins'
                                           )
        }
    ...
    for item in groups.values():
        warehouse = frappe.get_value(
            "Warehouse", item[0]["warehouse"], "almacen_ins")
        cantidad = 0
        valuation_rate = 0
        d = 0
        for i in item:
            cantidad += float(i["qty"]) * float(i["conversion"])
            valuation_rate += float(i["valuation_rate"]) / \
                float(i["conversion"])
            d += 1
        item_dic = {
...
def get_item(barcode):
    try:
        item = frappe.get_all(
            "Item", filters={"barcode": barcode}, fields=["name"])
        item_doc = frappe.get_doc("Item", item[0]['name'])
    except Exception:
        item_doc = False
    finally:
        return item_doc
...
@frappe.whitelist()
def validate_item(item_code, warehouse):
    result = {'cantidad_en_stock': None,
              'tipo_y_insumo': None, 'insumo_diario': None}
    result['cantidad_en_stock'] = frappe.get_list(
        "Stock Ledger Entry",
        {"item_code": item_code, "warehouse": warehouse},
        "qty_after_transaction"
    )
    result['tipo_y_insumo'] = frappe.get_value(
        "Item", item_code, ["item_type", "insumo"])
    try:
        result['insumo_diario'] = frappe.get_value(
            "Item", result['tipo_y_insumo'][1], "insumo_diario")
    except Exception:
        pass
    return result
