...
class MovimientodeInventario(Document):
	def on_submit(self):
		almacen_transito = frappe.get_value("Warehouse", self.source_warehouse, "almacen_transito")
...
def as_insumo(items):
	items_insumo = []
	for item in items:
		insumo = frappe.get_value("Item", item["item_code"], ["insumo", "conversion_ins"])
		uom = frappe.get_value("Item", insumo[0], "stock_uom")
		qty = item["qty"] * insumo[1]
		items_insumo.append({
			"item_code": insumo[0],
			"qty": qty,
			"basic_rate": frappe.get_value("Stock Ledger Entry", { "item_code": item["item_code"] }, "valuation_rate"),
			"uom": uom,
			"conversion_factor": 1,
			"stock_uom": uom,
			"transfer_qty": qty
...
def get_items(pedido, s_warehouse, t_warehouse):
	...
	for item in items_pedido:
		batch_no  = frappe.get_value("Stock Ledger Entry", { "warehouse": s_warehouse, "item_code": item.item_code }, "batch_no")
		items.append({
